...
## 2026-08-21 — 回测收益图增加坐标轴、0 轴参考线与悬停数据查看（累计收益率模式）

### 实施
- 回测图改为**累计收益率（%）**展示，替代原先的净值：
  - 策略收益率 = (净值 - 1) × 100%
  - 沪深300 收益率 = (基准归一化净值 - 1) × 100%
  - 0% 为基准，比净值更直观
- `EquityCurveWidget`：
  - 增加左侧 Y 轴刻度和底部 X 轴标签
  - 支持 0 轴参考线（收益率模式强制将 0 纳入 Y 轴范围并绘制虚线）
  - 鼠标悬停/点击显示十字线 + 各序列收益率浮框（带 %）
  - 支持日期标签：`setXLabels()`
- `BacktestResult` 增加 `equityDates`：
  - `BacktestEngine` 与净值曲线同步记录每个交易日
  - 回测面板将日期传给收益图，悬停/坐标轴可显示具体日期

### 验证
- Debug / Release 清洁重建通过（`--clean-first`，避免 ABI 陈旧对象）。
- 待手动复测：回测完成后收益图应显示 Y/X 轴、0% 参考线；鼠标在曲线上移动/点击可查看对应日期与策略/沪深300 收益率。
